...
use App\Http\Request\AuthorizationWebhookRequestParser;
...
/**
 * The inbound authorization webhook. Order matters: signature first
 * (cheap, fails fast), idempotency cache lookup next, then the command
 * handler. Caching the response shape after the handler runs means the
 * processor's retries see the exact same JSON the first call produced.
 */
final class AuthorizationWebhookController
{
    public function __construct(
        private readonly ProcessorWebhookSignatureVerifier $signatureVerifier,
        private readonly IdempotencyStore $idempotencyStore,
        private readonly AuthorizationWebhookRequestParser $parser,
        private readonly AuthorizeCardCommandHandler $handler,
    ) {
    }

    #[Route('/api/webhooks/authorization', name: 'webhook_authorization', methods: ['POST'])]
